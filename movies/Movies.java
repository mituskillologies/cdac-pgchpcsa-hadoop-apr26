 ...
 public static class Map extends Mapper<LongWritable, Text, IntWritable, FloatWritable> {
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
      String line = value.toString();  
			...
			int mnum = Integer.parseInt(words[0]);
			int rating = Integer.parseInt(words[2]);
			context.write(new IntWritable(mnum), new FloatWritable(rating));
    	}
 } 
    ...
    public void reduce(IntWritable key, Iterable<FloatWritable> values, Context context) 
      throws IOException, InterruptedException {	
			...
			for (FloatWritable rating : values) {
					float num = rating.get();
					total += 1;
					sum += num;
			}
			float avgRating = sum / (float) total;
			context.write(key, new FloatWritable(avgRating));
    }
 }
